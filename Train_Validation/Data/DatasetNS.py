...
class NS_Dataset2D:
    ...
    def __init__(self,
        ...
        self.cache_dir = self.data_dir + 'NS(uv)_cache_inter{}_test{}/'.format(t_interval, orders_test)
        self.param_dir = self.data_dir + 'NS(uv)_cache_inter{}_test{}/params.param'.format(t_interval, orders_test)
    ...
    def save_cache_file(self, split, test_order = None):
        ...
        cache_file = self.cache_dir + cache_name + '.NScache'
    ...
    def data_augmentation(self, u0, v0, p0, type):
        if type == 0:
            return u0, v0, p0
        else:
        ...
        for order in self.all_orders:
            ...
            for aug_i in range(8):
                ...
                for i in range(this_raw_data['u'].shape[0] // sampling_gap):
                    ...
                    u, v, p = self.data_augmentation(u, v, p, aug_i)
                    this_list.append({'u': u.copy().reshape((NG, NG)),
                                      'v': v.copy().reshape((NG, NG)),
                                      'p': p.copy().reshape((NG, NG))})
